/*
 * Author: Jonpas
 * Loads player using PersistenceClient extension.
 *
 * Arguments:
 * 0: Player <OBJECT>
 ...
 *
 * Example:
 * [player, "loaded"] call oda_persistence_fnc_playerLoadClient
 *
 * Public: No
 */
...
private _loadData = "oda_persistence_client" callExtension format ["%1|%2|%3", "loadPlayer", getPlayerUID _player, GVAR(isDebug)];
TRACE_1("Load Data Start",_loadData);

if (_loadData == "loaded") then {
    private _updateInfo = true;
    while {_updateInfo} do {
        private _loadData = "oda_persistence_client" callExtension format ["%1|%2|%3", "get", getPlayerUID _player, GVAR(isDebug)];
        ...
        if (_loadData == "error") then {
            ...
            private _codePacket = _loadData splitString "|";

            TRACE_1("Status", _codePacket);
            
            if ((_codePacket select 0) isEqualTo "0") then {
                TRACE_1("Code Packet",(_codePacket select 1));
                call (compile (_codePacket select 1));
